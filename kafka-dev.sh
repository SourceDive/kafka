...
# 启动Kafka Manager可视化界面
start_kafka_manager() {
    gum style --border=rounded --margin="1 2" --padding="2 4" --border-foreground=212 "🌐 启动Kafka Manager可视化界面"
...
# 启动Kafka Manager连接本地Kafka
start_kafka_manager_local() {
    gum style --border=rounded --margin="1 2" --padding="2 4" --border-foreground=212 "🔗 启动Kafka Manager连接本地Kafka集群"
...
show_menu() {
    ...
    local choice=$(gum choose "🚀 一键启动环境" "🛑 停止环境" "🧪 运行测试" "📊 查看状态" "🔧 重新构建" "🌐 启动可视化界面" "🔗 连接本地Kafka" "❌ 退出")
